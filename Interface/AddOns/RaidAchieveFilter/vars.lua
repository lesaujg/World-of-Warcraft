...
RAFdb.AchID[27] = {11930,12065,12129,11928,12067,11949,12030,11948,12046,11915,12257,11988,11989,11990,11991,11992,11993,11994,11995,11996,11997,11998,11999,12000,12001,12002}--Antorus
RAFdb.AchID[28] = {10555,10771,10830,10753,10663,10772,10755,10821,10822,10823,10824,10825,10826,10827,10818,10819,10820}--ENightmare
RAFdb.AchID[29] = {10678,10697,10742,10817,10704,10575,10754,10851,10699,10696,10840,10842,10843,10844,10847,10848,10846,10845,10849,10850,10829,10837,10838,10839}--Nighthold
RAFdb.AchID[30] = {11724,11696,11699,11676,11773,11674,11675,11683,11770,11767,11774,11775,11776,11777,11778,11779,11780,11781,11787,11788,11789,11790}--Tomb
RAFdb.AchID[31] = {11337,11386,11377,11394,11426,11396,11397,11398,11387}--ToValor
...
--Scenarios
if uFaction == "A" then 
	RAFdb.AchID[92] = {7261,7258,7257,7252,8310}--Brewing
	RAFdb.AchID[93] = {7989,7990,7991,7992,7993,7988}--Patience
	RAFdb.AchID[94] = {7273,7272,7271}--Arena
	RAFdb.AchID[95] = {8016,8017}--Assault
	RAFdb.AchID[96] = {8347,8314,8364}--Battle
	RAFdb.AchID[97] = {8329,8330,8316,8312}--Blood
	RAFdb.AchID[98] = {6931,6930,6923}--Brewmoon
	RAFdb.AchID[99] = {7275,7276,8368,7522,8311}--Crypt
	RAFdb.AchID[100] = {7987,7984,7986,8009}--Dagger
	RAFdb.AchID[101] = {8319,8317,8318}--DarkHeart
	RAFdb.AchID[102] = {7267,7266,7265}--Greenstone
	RAFdb.AchID[103] = {8012,8011,8010}--Lion's
	RAFdb.AchID[104] = {8295,8294,8327}--Ragefire
	RAFdb.AchID[105] = {7526,7527,7523}--Theramore
	RAFdb.AchID[106] = {7231,7232,7239,7249,7248}--Unga
	...
	for i = 1, 106 do RAFdb.MapName[i] = GetMapNameByID(RAFdb.MapID[i]) end
	RAFdb.MapName[94] = "Arena of Annihilation"-- (Proving Grounds)
	RAFdb.MapName[97] = "Blood in the Snow"-- (Dun Morogh)
	RAFdb.MapName[101] = "Dark Heart of Pandaria"-- (Vale of Eternal Blossoms)
	RAFdb.MapName[102] = "Greenstone Village"-- (Jade Forest)
	RAFdb.MapName[103] = "Lion's Landing"-- (Krasarang Wilds)
	RAFdb.MapName[105] = "Theramore's Fall"-- (Dustwallow Marsh)
else
	RAFdb.AchID[92] = {7261,7258,7257,7252,8310}--Brewing
	RAFdb.AchID[93] = {7989,7990,7991,7992,7993,7988}--Patience
	RAFdb.AchID[94] = {7273,7272,7271}--Arena
	RAFdb.AchID[95] = {8016,8017}--Assault
	RAFdb.AchID[96] = {8347,8315,8366}--Battle
	RAFdb.AchID[97] = {8329,8330,8316,8312}--Blood
	RAFdb.AchID[98] = {6931,6930,6923}--Brewmoon
	RAFdb.AchID[99] = {7275,7276,8368,7522,8311}--Crypt
	RAFdb.AchID[100] = {7987,7984,7986,8009}--Dagger
	RAFdb.AchID[101] = {8319,8317,8318}--DarkHeart
	RAFdb.AchID[102] = {8015,8014,8013}--Domination
	RAFdb.AchID[103] = {7267,7266,7265}--Greenstone
	RAFdb.AchID[104] = {8295,8294,8327}--Ragefire
	RAFdb.AchID[105] = {7529,7530,7524}--Theramore
	RAFdb.AchID[106] = {7231,7232,7239,7249,7248}--Unga
	...
	RAFdb.MapName[97] = "Blood in the Snow"--(Dun Morogh)
	RAFdb.MapName[101] = "Dark Heart of Pandaria"--(Vale of Eternal Blossoms)
	RAFdb.MapName[102] = "Domination Point"--(Krasarang Wilds)
	RAFdb.MapName[103] = "Greenstone Village"--(Jade Forest)
	RAFdb.MapName[105] = "Theramore's Fall"--(Dustwallow Marsh)
end
