...
secureFrame:SetScript("OnEvent",
	function(self, event)
		if event=="PLAYER_REGEN_ENABLED" then
			...
			if not self:IsShown() and self.owner then
...
local function AcquireButton( level )
	...
	if #buttons == 0 then
		...
		button = CreateFrame("Button", "ArkDewdrop30Button" .. numButtons, nil)
...
local function AcquireLevel(level)
	if not levels[level] then
		for i = #levels + 1, level, -1 do
			...
			local frame = CreateFrame("Button", "ArkDewdrop30Level" .. numLevels, nil)
			if i == 1 then
...
local function confirmPopup(message, func, ...)
	if not StaticPopupDialogs["ARKDEWDROP30_CONFIRM_DIALOG"] then
		StaticPopupDialogs["ARKDEWDROP30_CONFIRM_DIALOG"] = {}
	end
	local t = StaticPopupDialogs["ARKDEWDROP30_CONFIRM_DIALOG"]
	for k in pairs(t) do
		t[k] = nil
	end
	t.text = message
	...
	Dewdrop:Close()
	StaticPopup_Show("ARKDEWDROP30_CONFIRM_DIALOG")
end
...
function Dewdrop:AddLine(...)
	...
	button.disabled = info.isTitle or info.notClickable or info.disabled or (InCombat( ) and info.secure)
