...
--7 checkbox
	...
	rsccheckbox1:SetPoint("TOPLEFT", leftmarg, pii)
...
if rscflaskcheckb[7]==0 then
	...
	rscflaskt8 = rscframetoshowall4:CreateFontString()
	rscflaskt8:SetFont(GameFontNormal:GetFont(), rscfontsset[1])
	rscflaskt8:SetText(rsccheckrunes)
if rscflaskcheckb[8]==0 then
	rscflaskt8:SetText("|cffff0000"..rsccheckrunes.."|r")
end
	rscflaskt8:SetJustifyH("LEFT")
	rscflaskt8:SetPoint("TOPLEFT",184,-178)
	
	
	rscflasktfoodadd = rscframetoshowall4:CreateFontString()
	rscflasktfoodadd:SetFont(GameFontNormal:GetFont(), rscfontsset[1])
	rscflasktfoodadd:SetText(format(rscpartanonsfoodadd,"+300"))
	...
	rscflasktfoodadd2:SetText(format(rscpartanonsfoodadd,"+225"))
...
if #rscwillcheckthem>0 and rscflaskcheckb[6]==1 then
	for yy=1,#rscwillcheckthem do
		...
		for uu=1,#rscfoodtable do
			if UnitAura(rscwillcheckthem[yy],GetSpellInfo(rscfoodtable[uu])) then
				local _, _, _, _, _, _, expirationTime,_, _,_,spell_id,_,_,_,stats = UnitAura(rscwillcheckthem[yy],GetSpellInfo(rscfoodtable[uu]))
				if spell_id==rscfoodtable[uu] and (stats==nil or (stats and (stats>=rscMinFoodBuffAccepted or stats==0))) then
    ...
    if byl==0 and rscflaskcheckfoodadd and rscflaskcheckfoodadd==1 then
	  ...
	  -- some +100 buffs have 125 spellid, so check one more time
      for mm=1,#rscfoodtable do
        if UnitAura(rscwillcheckthem[yy],GetSpellInfo(rscfoodtable[mm])) then
          local _, _, _, _, _, _, expirationTime,_, _,_,spell_id = UnitAura(rscwillcheckthem[yy],GetSpellInfo(rscfoodtable[mm]))
          if spell_id==rscfoodtable[mm] then
            if expirationTime==nil or (expirationTime and expirationTime-GetTime()>rscminflaskgood*60) or expirationTime==0 then
              byl=100
              mm=100
            end
...
--проверяю на руны
if #rscwillcheckthem>0 and rscflaskcheckb[8]==1 then
	for yy=1,#rscwillcheckthem do
		local byl=0

		for uu=1,#rscrunestable do
			if UnitAura(rscwillcheckthem[yy],GetSpellInfo(rscrunestable[uu])) then
				local _, _, _, _, _, _, expirationTime,_, _,_,spell_id = UnitAura(rscwillcheckthem[yy],GetSpellInfo(rscrunestable[uu]))
				if spell_id==rscrunestable[uu] then
          if expirationTime==nil or (expirationTime and expirationTime-GetTime()>rscminflaskgood*60) or expirationTime==0 then
            byl=100
            uu=100
          end
		...
		if byl==0 then
			table.insert(rscnorunes,rscwillcheckthem[yy])
		elseif byl~=100 then
			table.insert(rscnorunes,rscwillcheckthem[yy].." ("..byl..")")
		end
	end
end
...
if #rscwillcheckthem>0 then
	for yy=1,#rscwillcheckthem do
		...
		if rscDoNotReportOldFlasks==nil then
...
--руны
if (rscnorunes and #rscnorunes>0) then
stringa=stringa.."|cffff0000"..rscflasktext69.."|r ("..#rscnorunes.."): "
	for eda=1,#rscnorunes do
		if eda>1 then
			stringa=stringa..", "
		end
		rsccoloraddfunc(rscnorunes[eda])

		local rname=""
		local rname2=""
		if string.find(rscnorunes[eda],"%(") then
			rname=string.sub(rscnorunes[eda],1,string.find(rscnorunes[eda],"%(")-2)
			rname2=string.sub(rscnorunes[eda],string.find(rscnorunes[eda],"%(")-1)
		else
			rname=rscnorunes[eda]
		end
...
--нет рун
if #rscnorunes>0 then
if #rscnorunes==#rscwillcheckthem and #rscnorunes>6 then
stringaRunes1=stringaRunes1.."No Runes: ("..#rscnorunes.."): "..string.upper(rscflasktext6).."!"
else
	stringaRunes1=stringaRunes1.."No runes: ("..#rscnorunes.."): "
	for eda=1,#rscnorunes do
		if string.len(stringaRunes1)>230 then
			if eda>1 and string.len(stringaRunes2)<2 then
				stringaRunes1=stringaRunes1..", "
			else
				stringaRunes2=stringaRunes2..", "
			end
			stringaRunes2=stringaRunes2..rscnorunes[eda]
		else
			if eda>1 then
				stringaRunes1=stringaRunes1..", "
			end
			stringaRunes1=stringaRunes1..rscnorunes[eda]
		end
	end
end
end
...
if manual=="auto" then
rscflaskdelayrep[2]=GetTime()+rscTimoutReportsInSec
end
...
SendAddonMessage("RSCaddon", "a50", "RAID")
	...
	--ыытест
	rscchatsendreports(chat,stringa1, stringa11, stringa2, stringa21, stringa22, stringatemp2, stringatemp21, stringa3, stringa31, stringa32, stringaRunes1, stringaRunes2, stringa4, stringa41)
	end
end
...
rscflaskdelayrep[1]=GetTime()+rscTimoutReportsInSec
rscchatfiltimefunc()
...
if #rscignorezone13[1]>0 then
	for ug=1,#rscignorezone13[1] do
		if rscignorezone13[1][ug]==curZoneID then
			rscignorezone13[2][ug]=GetMapNameByID(GetCurrentMapAreaID())
			mojn=0
		end
	end
end
...
elseif klass=="demonhunter" then rsccolor1="|CFFA330C9"
end
rsccolor2="|r"
...
if nn==1 then
	local rrr=0
	if #rscignorezone13[1]>0 then
		for i=1,#rscignorezone13[1] do
			if rscignorezone13[1][i]==curZoneID then
				rscignorezone13[2][i]=GetMapNameByID(GetCurrentMapAreaID())
				rrr=1
			end
		end
	end
	if rrr==0 then
		table.insert(rscignorezone13[2],GetMapNameByID(GetCurrentMapAreaID()))
		table.insert(rscignorezone13[1],curZoneID)
	...
	if #rscignorezone13[1]>0 then
		for i=1,#rscignorezone13[1] do
			if rscignorezone13[1][i]==curZoneID then
				...
				table.remove(rscignorezone13[1],i)
				table.remove(rscignorezone13[2],i)
				i=1000
			end
		end
	end
...
if nn==3 then
	local txt=""
	if #rscignorezone13[2]>0 then
		for i=1,#rscignorezone13[2] do
			txt=txt.."|cff00ff00"..rscignorezone13[2][i].."|r"
			if #rscignorezone13[2]>i then
				txt=txt..", "
			end
		end
	end
...
table.wipe(rscignorezone13[1])
table.wipe(rscignorezone13[2])
else
--тыкни еще раз!
