...
function indicatorAPI:SetIcon(texture)
	...
	local ofs = texture:match("^[Ii][Nn][Tt][Ee][Rr][Ff][Aa][Cc][Ee][\\/][Ii][Cc][Oo][Nn][Ss][\\/]") and (2.5/64) or (-2/64)
...
local CreateCooldown do
	...
	function CreateCooldown(parent, size)
		...
		for i=1,4 do
			cd[4+i] = cc("SetPoint", cc("SetTexture", parent:CreateTexture(nil, "ARTWORK", nil, 3), 1,1,1),
...
local CreateIndicator do
	...
	function CreateIndicator(name, parent, size, nested)
		...
		return setmetatable({[0]=e, cd=cd, cdText=cd.cdText,
			...
			veil = cc("SetTexture", cc("SetPoint", cc("SetSize", e:CreateTexture(nil, "ARTWORK", nil, 2), 60*size/64, 60*size/64), "CENTER"), 0, 0, 0),
