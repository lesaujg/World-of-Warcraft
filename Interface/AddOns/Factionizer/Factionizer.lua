----------------------------------------------------------			
-- Factionizer, a reputation management tool 6.01.01.07 15.06.10 42175.1638636574 --			
-------------------------------------------------------			
---------------------------			
...
FIZ_VNMBR = 601017	-- Number code for this version		
...
local quest_names = setmetatable({}, {			
	__index = function(t, id_num)		
		...
		elseif id_num == 9 then	
			quest_name = FIZ_TXT.djdq
...
--- f_am	FIZ_Printtest(english, localised,"map")		
	...
	if (FIZ_InitFaction(FIZ_GuildName,localised)) then		
...
---	FIZ_Initmapname(zone)		
	...
	rep = rep * FIZ_InitFactor(FIZ_IsHuman,FIZ_faction)		
	faction = string.lower(FIZ_InitFaction(FIZ_GuildName,faction))		
...
function FIZ_AddMob(faction, from, to, name, rep, zone, limit)			
	...
	for standing = from,to do		
		...
		add_info.data[count] = {}	
		local add_count=add_info.data[count]	
		add_count.name = FIZ_Initmobname(name)	
		add_count.rep = rep	
		add_count.maxStanding = to	
		add_count.zone = FIZ_Initmapname(zone)	
		if ((standing == to) and limit) then	
			add_count.limit = limit
		end	
...
function FIZ_AddQuest(faction, from, to, name, rep, itemList, limitType)			
	...
	for standing = from,to do		
		...
		add_info.data[count] = {}	
		local add_count=add_info.data[count]	
		add_count.name = quest_names[name]	
		add_count.rep = rep	
		add_count.maxStanding = to	
		if (itemList) then	
			if (itemList == "nil") then
				...
				for item in pairs(itemList) do			
					add_count.items[FIZ_Inititemname(item)] = itemList[item]		
...
function FIZ_AddInstance(faction, from, to, name, rep, heroic)			
	...
	for standing = from,to do		
		...
		add_info.data[count] = {}	
		local add_count=add_info.data[count]	
		add_count.name = FIZ_Initmapname(name)	
		add_count.rep = rep	
		add_count.maxStanding = to	
		add_count.level = (isheroic and	
			 FIZ_TXT.heroic
		or	
			FIZ_TXT.normal
		)	
...
function FIZ_Update_Tooltip(x, l1,r1)			
	x=x+1		
...
--- ddd							FIZ_Update_Tooltip(x, FIZ_TXT.mode, fg_sid_x_d.level)
					...
					local fg_sid_x_d=fg_sid_x.data[i]		
						if (not fg_sid_x_d.limit or (normCurrent < fg_sid_x_d.limit)) then	
							...
							if (fg_sid_x_d.zone) then
								...
								local FUL_I_TD = FUL_I.tooltipDetails			
								local x = 0			
								FUL_I_TD[x], x = FIZ_Update_Tooltip(x, FIZ_TXT.mob2, fg_sid_x_d.name)			
								FUL_I_TD[x], x = FIZ_Update_Tooltip(x, FIZ_TXT.location, fg_sid_x_d.zone)			
								FUL_I_TD[x], x = FIZ_Update_Tooltip(x, FIZ_TXT.reputation, FUL_I.rep)			
								FUL_I_TD[x], x = FIZ_Update_Tooltip(x, FIZ_TXT.toDo, FUL_I.times)			
								FUL_I_TD[x], x = FIZ_Update_Tooltip(x, " ", " ")			
								FUL_I_TD[x] = FIZ_Update_Tooltip(x, FIZ_TXT.maxStanding, _G["FACTION_STANDING_LABEL"..fg_sid_x_d.maxStanding])			
								FUL_I_TD.count = x			
							else
								...
								FUL_I_TD[x], x = FIZ_Update_Tooltip(x, FIZ_TXT.FIZ_TXT.toDo, FUL_I.times)			
				...
				if (fg_sid.quests and FIZ_Data.ShowQuests) then			
					...
					if (showQuest) then		
						if (not fg_sid_x_d.limit or (normCurrent < fg_sid_x_d.limit)) then	
							...
							local FUL_I_TD = FUL_I.tooltipDetails
							local x = 0
							FUL_I_TD[x], x = FIZ_Update_Tooltip(x, FIZ_TXT.quest2, FUL_I.name)
							FUL_I_TD[x], x = FIZ_Update_Tooltip(x, FIZ_TXT.reputation, FUL_I.rep)
							FUL_I_TD[x], x = FIZ_Update_Tooltip(x, FIZ_TXT.toDo, FUL_I.times)
							...
							if (fg_sid_x_d.items) then
								...
								FUL_I_TD[x], x = FIZ_Update_Tooltip(x, " ", " ")			
								FUL_I_TD[x], x = FIZ_Update_Tooltip(x, FIZ_TXT.itemsRequired, " ")			
								-- quest In log?			
								FUL_I.lowlight = nil			
								...
								for item in pairs(fg_sid_x_d.items) do			
									...
									FUL_I_TD[x], x = FIZ_Update_Tooltip(x, fg_sid_x_d.items[item].."x", item)		
								...
								if (currentQuestTimesBag > 0) then			
									...
									FUL_I_TD[x], x = FIZ_Update_Tooltip(x, " ", " ")		
									FUL_I_TD[x], x = FIZ_Update_Tooltip(x, FIZ_TXT.inBag, " ")		
									FUL_I_TD[x], x = FIZ_Update_Tooltip(x, FIZ_TXT.turnIns, string.format("%d", FUL_I.currentTimesBag))		
									FUL_I_TD[x], x = FIZ_Update_Tooltip(x, FIZ_TXT.reputation, string.format("%d", FUL_I.currentRepBag))		
								else			
								...
								if (currentQuestTimesBagBank > 0) then			
									...
									FUL_I_TD[x], x = FIZ_Update_Tooltip(x, " ", " ")		
									FUL_I_TD[x], x = FIZ_Update_Tooltip(x, FIZ_TXT.inBagBank, " ")		
									FUL_I_TD[x], x = FIZ_Update_Tooltip(x, FIZ_TXT.turnIns, string.format("%d", FUL_I.currentTimesBagBank))		
									FUL_I_TD[x], x = FIZ_Update_Tooltip(x, FIZ_TXT.reputation, string.format("%d", FUL_I.currentRepBagBank))		
								else			
									...
									if (title and not header) then		
										if string.find(string.lower(fg_sid_x_d.name), string.lower(title)) then	
											...
											if (complete) then
												...
												FUL_I_TD[x], x = FIZ_Update_Tooltip(x, " ", " ")			
												FUL_I_TD[x], x = FIZ_Update_Tooltip(x, FIZ_TXT.questCompleted, " ")			
												FUL_I_TD[x], x = FIZ_Update_Tooltip(x, FIZ_TXT.reputation, string.format("%d", FUL_I.currentRepQuest))			
											else
												FUL_I.lowlight = true			
												FUL_I.name = FUL_I.name..FIZ_QUEST_ACTIVE_COLOUR.." ("..FIZ_TXT.active..")|r"			
											end
				...
				if ((sum > 0) and (count > 1)) then			
					...
					FUL_I_TD[x], x  = FIZ_Update_Tooltip(x, FIZ_TXT.timesToDo, FUL_I.times)		
					FUL_I_TD.count = x-1		
					FUL_I.tooltipDetails = FUL_I_TD		
					FIZ_UpdateList[index] = FUL_I		
					index = index + 1		
				...
				for i = 0, fg_sid_x.count do			
					...
					if (not fg_sid_x_d.limit or (normCurrent < fg_sid_x_d.limit)) then		
						...
						if (fg_sid_x_d.items) then	
							...
							FUL_I_TD[x], x = FIZ_Update_Tooltip(x, FUL_I.name, " ")
							FUL_I_TD[x], x = FIZ_Update_Tooltip(x, " ", " ")
							FUL_I_TD[x], x = FIZ_Update_Tooltip(x, FIZ_TXT.itemsRequired, " ")
				...
				if (fg_sid.general and FIZ_Data.ShowGeneral) then			
					...
					for i = 0, fg_sid_x.count do		
						...
						if (not fg_sid_x_d.limit or (normCurrent < fg_sid_x_d.limit)) then	
							...
							local FUL_I_TD = FUL_I.tooltipDetails
							local x = 0
							FUL_I_TD[x], x = FIZ_Update_Tooltip(x, FIZ_TXT.general2, fg_sid_x_d.name)
							FUL_I_TD[x], x = FIZ_Update_Tooltip(x, FIZ_TXT.reputation, FUL_I.rep)
							FUL_I_TD[x], x = FIZ_Update_Tooltip(x, FIZ_TXT.timesToRun, FUL_I.times)
							FUL_I_TD[x], x = FIZ_Update_Tooltip(x, " ", " ")
							if (fg_sid_x_d.tipList) then
								for tip in pairs(fg_sid_x_d.tipList) do			
			
									FUL_I_TD[x], x = FIZ_Update_Tooltip(x, tip, fg_sid_x_d.tipList[tip])		
								end			
								FUL_I_TD[x], x = FIZ_Update_Tooltip(x, " ", " ")			
...
function FIZ_Quest_Items(itemQuest, toDo, x, itemIndex, currentQuestTimesBag, currentQuestTimesBagBank, item)			
	for item in pairs(itemQuest) do		
		local itemNeed = itemQuest[item]	
			
		QuestItem = {	
			type = "",
			times = (itemsNeed * toDo).."x",
			rep = nil,
			index = itemIndex,
			belongsTo = index,
			hasList = nil,
			listShown = nil,
			isShown = listShown,
			name = item.." ("..itemNeed.."x)",
		}	
		local toolTip = {}	
			
		toolTip[x], x = FIZ_Update_Tooltip(x, itemNeed.."x", item)	
			
		if (FIZ_ItemsCarried and FIZ_ItemsCarried[item]) then	
			if ((FIZ_ItemsCarried[item] >= itemsNeed) and (itemsNeed > 0)) then
				QuestItem.currentTimesBag = floor(FIZ_ItemsCarried[item] / itemsNeed)			
				QuestItem.name = QuestItem.name..FIZ_BAG_COLOUR.." ["..FIZ_ItemsCarried[item].."x]|r"			
				if (currentQuestTimesBag == -1) then			
					-- first items for this quest --> take value		
				...
				-- not enough of this item for quest -> set to 0			
				currentQuestTimesBag = 0			
				QuestItem.name = QuestItem.name.." ["..FIZ_ItemsCarried[item].."x]"			
			end
			if (FIZ_Data.Bank and
				FIZ_Data.Bank[FIZ_Realm.." - "..FIZ_Player] and			
				FIZ_Data.Bank[FIZ_Realm.." - "..FIZ_Player][item]) then			
				local total = FIZ_Data.Bank[FIZ_Realm.." - "..FIZ_Player][item] + FIZ_ItemsCarried[item]			
				if ((total >= itemsNeed) and (itemsNeed > 0)) then			
					QuestItem.name = QuestItem.name..FIZ_BAG_BANK_COLOUR.." ["..total.."x]|r"		
					QuestItem.currentTimesBagBank = floor(total / itemsNeed)		
					if (currentQuestTimesBagBank == -1) then		
						-- first items for this quest --> take value	
					...
					-- not enough of this item for quest -> set to 0		
					currentQuestTimesBagBank = 0		
					QuestItem.name = QuestItem.name.." ["..total.."x]"		
				end			
			else
				-- none of this carried In bank			
			end
		...
		FIZ_UpdateList[itemIndex] = QuestItem	
		itemIndex = itemIndex + 1	
	end		
	return QuestItem, toolTip, x, itemIndex, currentQuestTimesBag, currentQuestTimesBagBank		
end			
