...
L["|cffff0000Your TSM addons are out of date!|r Please DO NOT report this error, but instead update your TSM addons from here:"] = "|cffff0000你的TSM插件過期了!|r 無需回報這項錯誤, 你可以從下列網址更新TSM插件:"
L["|cffffff00DO NOT report this as an error to the developers.|r If you require assistance with this, join our IRC channel or make a post on the TSM forums instead."] = "不需回報這個錯誤給開發者.如果你需要協助可以加入IRC頻道或是在TSM的論壇上發表文章"
L["|cffffff00Important Note:|r An update is available for %s. You should update as soon as possible to ensure TSM continues to function properly."] = "|cffffff00重點提示:|r %s 有新的更新檔了. 你應該盡速更新確保TSM運作正常."
...
L["If checked, after moving items using BankUI at the bank your bank bags will be automatically sorted / re-stacked."] = "如勾選確認,當你使用BankUI移動銀行內的物品後,銀行包包內的物品將自動的重新分類/堆疊"
-- L["If checked, after moving items using BankUI at the bank your reagent bank bags will be automatically sorted / re-stacked."] = ""
L["If checked, after moving items using BankUI your bags will be automatically sorted / re-stacked."] = "如勾選確認,當你使用BankUI移動包包內的物品後,包包內的物品將自動的重新分類/堆疊"
L["If checked, all operations will be exported, including all of their settings."] = "沒問題的話,所有操作將被輸出,包含所有相關的設定"
...
L["If checked, TSM will provent WoW from closing the auction house frame when other UI frames are opened."] = "如勾選確認,當其他UI視窗開啟時,TSM會防止WOW關掉拍賣場的視窗" -- Needs review
...
L["Incoming Auction Sales"] = "正在進行的拍賣" -- Needs review
L["Info"] = "資訊"
...
L["Item"] = "物品"
-- L["Item info for %d items is still loading and may impact TSM functionality until complete."] = ""
...
L["Looks like TradeSkillMaster has encountered an error. Please help the author fix this error by copying the entire error below and following the instructions for reporting lua errors listed at the following URL:"] = "TSM發生了一個錯誤,請幫開發者修正這個錯誤.請複製完整的錯誤訊息,在下列網址中依步驟回報,謝謝"
...
L["Skip"] = "略過"
L["Skipping %s operations as the module is not loaded."] = "當模組沒被載入時,略過%s的操作"
...
L["TradeSkillMaster tooltip options for |cff99ffff%s|r have changed and therefore been reset to their default values."] = "|cff99ffff%s|r 的TSM提示參數已經被更動了,將回復成起始值~" -- Needs review
-- L["Transform Value:"] = ""
