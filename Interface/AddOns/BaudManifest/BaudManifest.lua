...
function BaudManifestDisplay_OnLoad(self)
  ...
  local function CheckMimick(self, ...)
    HookData[self:GetName()]:SetChecked(...);
  end
  ...
  local Edge = 10;
  if (ID == 1) then
    Button = CreateFrame("CheckButton", BagFrame:GetName() .. "BackpackButton", BagFrame, "ItemButtonTemplate");
    ...
    HookData[Inherit:GetName()] = Button;
    hooksecurefunc(Inherit, "SetChecked", CheckMimick);

    for Bag = 1, 4 do
      --Bag name length is restricted.  for this one, the ID is set automatically.
      BagSlot = CreateFrame("CheckButton", "BaudMInveBag" .. (Bag - 1) .. "Slot", BagFrame, "BagSlotButtonTemplate");
      BagSlot:SetPoint("TOP", 0, -Edge - 42 - (Bag - 1) * 32);
      Frame = "CharacterBag" .. (Bag - 1) .. "Slot";
      HookData[Frame] = BagSlot;
      hooksecurefunc(_G[Frame], "SetChecked", CheckMimick);
    end
  ...
  elseif (ID == 2) then
    for Bag = 1, NUM_BANKBAGSLOTS do
      --Bag name length is restricted
      BagSlot = CreateFrame("Button", "BaudMBankBag" .. Bag, BagFrame, "BankItemButtonBagTemplate");
...
hooksecurefunc("OpenStackSplitFrame", function(maxStack, parent, anchor, anchorTo)
