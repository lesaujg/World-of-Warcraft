...
function addon:OnInitialize()
    ...
    self:ScheduleTimer(function()
        ...
        if self.ac then
            self.acTabNum = AltCraftFrame:AddTab(AltCraftMDLTabFrame, 'Mount Drop List')
        end
    end, 0.5)
end
...
function addon:GetItemSourceInfo(itemSource)
	...
	elseif itemSource.type == 'expeditions' then
		zoneName = "Island Expeditions"
	end

    local npcName
    if itemSource.type == 'special' and itemSource.subtype ~= 'summoned' and itemSource.subtype ~= 'warfront' and itemSource.type ~= 'expeditions' then
    ...
    local raidSaveZone = MDL_DB_ZONES[itemSource.zone_id] and MDL_DB_ZONES[itemSource.zone_id].raid and LBZ[MDL_DB_ZONES[itemSource.zone_id].raid] or zoneName
    local raidSaveBoss = MDL_DB_BOSSES[itemSource.npc_id] and MDL_DB_BOSSES[itemSource.npc_id].raid and LBB[MDL_DB_BOSSES[itemSource.npc_id].raid] or npcName
...
function addon:BuildTooltipData()
    ...
    local normalItems, raidItems, worldItems, questItems, summonedItems, warfrontItems, expeditionsItems, zoneItems = {}, {}, {}, {}, {}, {}, {}, {}
            ...
            for _, itemSource in pairs(itemData.from) do
                if not itemSource.faction or itemSource.faction == playerFaction then
                    if itemSource.level <= playerLevel then
						...
						elseif itemSource.subtype == 'summoned' or itemSource.subtype == 'warfront' or itemSource.type == 'expeditions' then
                            add = 1
                        ...
                        if add then
                            local zoneData = {
                                ...
                                isCurrent = playerZoneName == (MDL_DB_ZONES[itemSource.zone_id] and MDL_DB_ZONES[itemSource.zone_id].map and LBZ[MDL_DB_ZONES[itemSource.zone_id].map] or zoneName),
                            }
