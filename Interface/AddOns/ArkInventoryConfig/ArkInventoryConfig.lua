			...
			args = {
				...
				restack = {
					order = 120,
						...
						desc = ArkInventory.Localise["CONFIG_SYSTEM_RESTACK_TEXT"],
						type = "toggle",
						get = function( info )
							return ArkInventory.db.global.option.restack.cleanup
						end,
						set = function( info, v )
							ArkInventory.db.global.option.restack.cleanup = v
						end,
					},
				tooltip = {
					order = 200,
...
function ArkInventory.ConfigInternalControlSettings( path )
	...
	local args1 = {
		monitor = {
			...
			get = function( info )
				local loc_id = ConfigGetNodeArg( info, #info - 1 )
				return ArkInventory.db.global.player.data[ArkInventory.Global.Me.info.player_id].monitor[loc_id]
			end,
			set = function( info, v )
				local loc_id = ConfigGetNodeArg( info, #info - 1 )
				ArkInventory.db.global.player.data[ArkInventory.Global.Me.info.player_id].monitor[loc_id] = v
			end,
		},
		save = {
			order = 200,
			...
			get = function( info )
				local loc_id = ConfigGetNodeArg( info, #info - 1 )
				return ArkInventory.db.global.player.data[ArkInventory.Global.Me.info.player_id].save[loc_id]
			end,
			set = function( info, v )
				local loc_id = ConfigGetNodeArg( info, #info - 1 )
				ArkInventory.db.global.player.data[ArkInventory.Global.Me.info.player_id].save[loc_id] = v
			end,
		},
		notifyerase = {
			order = 300,
		...
		override = {
			...
			get = function( info )
				local loc_id = ConfigGetNodeArg( info, #info - 1 )
				return ArkInventory.db.global.player.data[ArkInventory.Global.Me.info.player_id].control[loc_id]
			end,
			set = function( info, v )
				local loc_id = ConfigGetNodeArg( info, #info - 1 )
		...
		window = {
			...
			args = {
				...
				height = {
					order = 410,
					name = ArkInventory.Localise["HEIGHT"],
					desc = ArkInventory.Localise["CONFIG_SETTINGS_FRAME_HEIGHT_TEXT"],
					type = "range",
					min = 200,
					max = 2000,
					step = 40,
					get = function( info )
						local loc_id = ConfigGetNodeArg( info, #info - 2 )
						return ArkInventory.LocationOptionGetReal( loc_id, "window", "height" )
					end,
					set = function( info, v )
						local v = math.floor( v )
						...
						local loc_id = ConfigGetNodeArg( info, #info - 2 )
						if v ~= ArkInventory.LocationOptionGetReal( loc_id, "window", "height" ) then
							ArkInventory.LocationOptionSetReal( loc_id, "window", "height", v )
							ArkInventory.Frame_Main_Generate( nil, ArkInventory.Const.Window.Draw.Recalculate )
						end
					end,
				},
					...
					args = {
						method = {
							...
							get = function( info )
								...
								local v = ArkInventory.LocationOptionGetReal( loc_id, "sort", "method" )
								return v
							end,
							set = function( info, v )
								local loc_id = ConfigGetNodeArg( info, #info - 3 )
								ArkInventory.LocationOptionSetReal( loc_id, "sort", "method", v )
...
--[[
							...
							get = function( info )
								local loc_id = ConfigGetNodeArg( info, #info - 3 )
								return ArkInventory.LocationOptionGetReal( loc_id, "slot", "empty", "first" ) or 0
							end,
							set = function( info, v )
								local loc_id = ConfigGetNodeArg( info, #info - 3 )
								local v = math.floor( v )
...
function ArkInventory.ConfigInternalSorting( )
	...
	path.method = {
		order = 100,
		type = "group",
		name = ArkInventory.Localise["CONFIG_SORTING_METHODS"],
		args = {
			add = {
				order = 100,
				name = ArkInventory.Localise["CONFIG_SORTING_ADD"],
				desc = ArkInventory.Localise["CONFIG_SORTING_ADD_TEXT"],
				type = "input",
				get = function( )
					return ""
				end,
				set = function( info, v )
					ArkInventory.SortingMethodAdd( v )
				end,
			},
			sortbyname = {
				order = 200,
				...
				type = "execute",
				func = function( )
					ArkInventory.Global.Options.SortKeyMethodSort = true
					ArkInventory.ConfigInternalSorting( )
				end,
			},
			sortbynumber = {
				...
				type = "execute",
				func = function( )
					ArkInventory.Global.Options.SortKeyMethodSort = false
					ArkInventory.ConfigInternalSorting( )
				end,
			},
			reload = {
	...
	ArkInventory.ConfigInternalSortingMethod( path.method.args )
	
end

function ArkInventory.ConfigInternalSortingMethod( path )
	...
	local args3 = {
		enabled = {
			...
			disabled = function( info )
				...
				return ArkInventory.db.global.option.sort.method.data[id].system
			end,
			name = function( info )
				local id = ConfigGetNodeArg( info, #info - 3 )
				local key = ConfigGetNodeArg( info, #info - 1 )
				local checked = ArkInventory.db.global.option.sort.method.data[id].active[key]
			...
			get = function( info )
				local id = ConfigGetNodeArg( info, #info - 3 )
				local key = ConfigGetNodeArg( info, #info - 1 )
				return ArkInventory.db.global.option.sort.method.data[id].active[key]
			end,
			set = function( info, v )
				local id = ConfigGetNodeArg( info, #info - 3 )
				local key = ConfigGetNodeArg( info, #info - 1 )
				ArkInventory.db.global.option.sort.method.data[id].active[key] = v
				ArkInventory.LocationSetValue( nil, "resort", true )
				ArkInventory.Frame_Main_Generate( nil, ArkInventory.Const.Window.Draw.Resort )
			end,
		},
		up = {
			...
			disabled = function( info )
				...
				local key = ConfigGetNodeArg( info, #info - 1 )
				local checked = ArkInventory.db.global.option.sort.method.data[id].active[key]
				return ArkInventory.db.global.option.sort.method.data[id].system or not checked
			end,
			func = function( info )
				local id = ConfigGetNodeArg( info, #info - 3 )
				local key = ConfigGetNodeArg( info, #info - 1 )
				ArkInventory.SortingMethodMoveUp( id, key )
				ArkInventory.Frame_Main_Generate( nil, ArkInventory.Const.Window.Draw.Resort )
			end,
		},
		down = {
			...
			func = function( info )
				local id = ConfigGetNodeArg( info, #info - 3 )
				local key = ConfigGetNodeArg( info, #info - 1 )
				ArkInventory.SortingMethodMoveDown( id, key )
				ArkInventory.Frame_Main_Generate( nil, ArkInventory.Const.Window.Draw.Resort )
			end,
		},
	}
	...
	local args1 = {
		ren = {
			order = 100,
			name = ArkInventory.Localise["RENAME"],
			desc = ArkInventory.Localise["CONFIG_SORTING_RENAME_TEXT"],
			type = "input",
			disabled = function( info )
				local id = ConfigGetNodeArg( info, #info - 1 )
				return ArkInventory.db.global.option.sort.method.data[id].system
			...
			get = function( info )
				local id = ConfigGetNodeArg( info, #info - 1 )
				return ArkInventory.db.global.option.sort.method.data[id].name
			end,
			set = function( info, v )
				local id = ConfigGetNodeArg( info, #info - 1 )
				ArkInventory.db.global.option.sort.method.data[id].name = string.trim( v )
				ArkInventory.ConfigInternalSorting( )
			end,
		},
		del = { 
			order = 200,
			name = ArkInventory.Localise["DELETE"],
			desc = ArkInventory.Localise["CONFIG_SORTING_DELETE_TEXT"],
			type = "execute",
			disabled = function( info )
				local id = ConfigGetNodeArg( info, #info - 1 )
				return ArkInventory.db.global.option.sort.method.data[id].system
			...
			func = function( info )
				local id = ConfigGetNodeArg( info, #info - 1 )
				ArkInventory.SortingMethodDelete( id, "DELETE" )
				ArkInventory.ConfigInternalSorting( )
			end,
		},
		style = {
			...
			get = function( info )
				local id = ConfigGetNodeArg( info, #info - 1 )
				local v = ArkInventory.db.global.option.sort.method.data[id].bagslot
				if v then
					v = 1
				else
					v = 2
			...
			set = function( info, v )
				...
				ArkInventory.db.global.option.sort.method.data[id].bagslot = v
				ArkInventory.LocationSetValue( nil, "resort", true )
				ArkInventory.Frame_Main_Generate( nil, ArkInventory.Const.Window.Draw.Resort )
			end,
		},
		direction = {
			...
			get = function( info )
				local id = ConfigGetNodeArg( info, #info - 1 )
				local v = ArkInventory.db.global.option.sort.method.data[id].ascending
				if v then
					v = 1
				else
					v = 2
			...
			set = function( info, v )
				...
				local id = ConfigGetNodeArg( info, #info - 1 )
				ArkInventory.db.global.option.sort.method.data[id].ascending = v
				ArkInventory.Frame_Main_Generate( nil, ArkInventory.Const.Window.Draw.Resort )
			end,
		},
		order = {
			...
			hidden = function( info )
				local id = ConfigGetNodeArg( info, #info - 1 )
				return ArkInventory.db.global.option.sort.method.data[id].bagslot
			end,
			args = args2,
		},
		reverse = {
			...
			hidden = function( info )
				local id = ConfigGetNodeArg( info, #info - 1 )
				return ArkInventory.db.global.option.sort.method.data[id].bagslot or not ArkInventory.db.global.option.sort.method.data[id].active.name
			end,
			get = function( info )
				local id = ConfigGetNodeArg( info, #info - 1 )
				return ArkInventory.db.global.option.sort.method.data[id].reversed
			end,
			set = function( info, v )
				local id = ConfigGetNodeArg( info, #info - 1 )
				ArkInventory.db.global.option.sort.method.data[id].reversed = v
				ArkInventory.Frame_Main_Generate( nil, ArkInventory.Const.Window.Draw.Resort )
			end,
		},
	}
	...
	for id, data in pairs( ArkInventory.db.global.option.sort.method.data ) do
		...
		if data.used then
			...
			local n = ""
			if ArkInventory.Global.Options.SortKeyMethodSort then
