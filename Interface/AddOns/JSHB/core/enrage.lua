...
local enrageTexture = "Interface\\Icons\\Spell_Nature_Drowsy" -- Tranquilizing Shot
...
function JSHB.SetupEnrageModule(lockName)
	...
	if ( (not lockName) or (lockName == "MOVER_ENRAGEALERT") ) then
		...
		JSHB.F.EnrageAlert:SetScript(
			"OnUpdate",
			function(self, elapsed)
				self.updateTimer = self.updateTimer + elapsed
				if self.updateTimer <= ENRAGE_UPDATEINTERVAL then
					return
				else
					self.updateTimer = 0
				end
	...
	if ( (not lockName) or (lockName == "MOVER_ENRAGEREMOVABLES") ) then
		...
		if (JSHB.db.profile.enrage.removednotify == true) then
			...
			JSHB.F.EnrageAlert:SetScript("OnEvent",
				function(self, event, ...)
		...
		-- Removables setup
		if not JSHB.db.profile.enrage.enableremovables then 
			return
		end
		if (not (JSHB.playerClass == "HUNTER") ) then
			return
		end
		...
		for i=1,40 do
			...
			local xPos = ((JSHB.db.profile.enrage.iconsizeremovables + (JSHB.GetFrameOffset(JSHB.F.EnrageAlertRemovables.buffFrames[i], "LEFT", 1) + JSHB.GetFrameOffset(JSHB.F.EnrageAlertRemovables.buffFrames[i], "RIGHT", 1) + 2) ) * mod(i-1, 8) )
			local yPos = (JSHB.db.profile.enrage.iconsizeremovables +  (JSHB.GetFrameOffset(JSHB.F.EnrageAlertRemovables.buffFrames[i], "TOP", 1) + JSHB.GetFrameOffset(JSHB.F.EnrageAlertRemovables.buffFrames[i], "BOTTOM", 1) + 2) ) * floor( (i-1) / 8)
			...
			if JSHB.db.profile.enrage.removablestips then
				JSHB.F.EnrageAlertRemovables.buffFrames[i]:SetScript(
					"OnEnter",
					function(self)
		...
		JSHB.F.EnrageAlertRemovables:SetScript(
			"OnUpdate",
			function(self, elapsed, ...)
				self.updateTimer = self.updateTimer + elapsed		
				if self.updateTimer < ENRAGE_UPDATEINTERVAL then
				...
				if (not UnitCanAttack("player", "target") ) or (JSHB.db.profile.enrage.removablespvponly and (JSHB.GetGroupType() ~= "ARENA") and (JSHB.GetGroupType() ~= "BATTLEGROUND") ) then
					for i=1,40 do
						JSHB.F.EnrageAlertRemovables.buffFrames[i].spellID = 0
						JSHB.F.EnrageAlertRemovables.buffFrames[i]:Hide()
					end			
