	...
	if JSHB.db.profile.indicators.aspect_enable then
		...
		local function DoAspectUpdate(self)
			...
			if InCombatLockdown() then
				if (hasAspect == true) and (not UnitInVehicle("player")) and (not C_PetBattles.IsInBattle()) then
					JSHB.F.AspectIndicator:SetAlpha((JSHB.db.profile.indicators.aspect_matchbaralpha and JSHB.db.profile.focusbar.enabled) and JSHB.F.FocusBar:GetAlpha() or 1)
				...
				if(hasAspect == true) and (not JSHB.db.profile.indicators.aspect_onlycombat) and (not UnitInVehicle("player")) and (not C_PetBattles.IsInBattle()) then
					JSHB.F.AspectIndicator:SetAlpha( (JSHB.db.profile.indicators.aspect_matchbaralpha and JSHB.db.profile.focusbar.enabled) and JSHB.F.FocusBar:GetAlpha() or 1)
					AutoCastShine_AutoCastStop(JSHB.F.AspectIndicator.shine)
				else
					JSHB.F.AspectIndicator:SetAlpha(0)
